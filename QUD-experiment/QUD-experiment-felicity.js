...
var examples_specific =	[	{subject:'mouse',number:'singular',definiteness:'indefinite',animacy:'animate',
							...
							tense:'past',context:'specific',sentence:'My father used to drive a bus.'}]
							
var examples_generic = [	{subject:'sword',number:'plural',definiteness:'indefinite',animacy:'inanimate',
							tense:'present',context:'generic',sentence:'The moon is made of green cheese.'},
...
function make_explanation(subject, rating){
	var explanation;
	if (subject == 'mouse' && rating <= 3){
		explanation = "You should have indicated that this is a reasonable response. Click the button below one more time to continue."
	}
	else if (subject == 'mouse' && rating > 3){
		explanation = "Good job. This is a fairly reasonable response. Click the button below one more time to continue."
	}
	else if (subject == 'bus' && rating < 3){
		explanation = "Good job. This response is fairly unreasonable. Click the button below one more time to continue."
	}
	else if (subject == 'bus' && rating >= 3){
		explanation = "You should have indicated that this is an unreasonable response, since it doesn't say anything about what is currently happening. Click the button below one more time to continue."
	}
	else if (subject == 'sword' && rating < 3){
		explanation = "Good job. This is a fairly unreasonable response. Click the button below one more time to continue."
	}
	else if (subject == 'sword' && rating >= 3){
		explanation = "You should have indicated that this is an unreasonable response, since it is completely unrelated to the context. Click the button below one more time to continue."
	}
	else if (subject == 'zebra' && rating <= 3){
		explanation = "You should have indicated that this is a reasonable response. Click the button below one more time to continue." 
	}
	else if (subject == 'zebra' && rating > 3){
		explanation = "Good job. This is a fairly reasonable response. Click the button below one more time to continue."
...
function make_sentence_order(){
	sentence_order = []
	shuffled_specific = shuffle(examples_specific);
	shuffled_generic = shuffle(examples_generic);
	flip = random(0,1)
	if (flip == 0){
		sentence_order.push(shuffled_specific[0]);
		sentence_order.push(shuffled_generic[0]);
		sentence_order.push(shuffled_specific[1]);
		sentence_order.push(shuffled_generic[1]);
	} else {
		sentence_order.push(shuffled_generic[0]);
		sentence_order.push(shuffled_specific[0]);
		sentence_order.push(shuffled_generic[1]);
...
var experiment = {
   ...
   // or "if (stimuli.length == 0) {...}", etc: test whether it's time to end the experiment
			...
			qdata.type = 'example'
			var naturalness_response;
			var naturalness_answered;
			var explanation = false;
			
			if (context == 'bare'){
				$('#context').hide()
				$('.question_text_1').html('Would ')
				$('.question_text_2').html(' as something to say at the start of a conversation')
			}
			else{
				$('#context').show()
				$('.question_text_1').html('Does ')
				$('.question_text_2').html(' given the context')
			}
        	...
        	$(".rating").change(function() {
				naturalness_response = $(this).attr("value");
				naturalness_answered = true;
        	});
			
			$(".continue").click(function() {
	    		var clickTime = (new Date()).getTime();
	    		if (!naturalness_answered) { // test for answer meeting relevant parameters -- e.g., all questions answered
	    			// if no, show some text saying so and ask them to answer appropriately
	    			$(".no_answer").show();
	    		} 
	    		else if (naturalness_answered && !explanation){
	    			var explanation_text = make_explanation(subject,parseInt(naturalness_response));
	    			...
	    			qdata.naturalness = naturalness_response;
